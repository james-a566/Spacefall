...
ENEMY_DIE_FLASH_FR  = 8    ; death flash length
EN_A = $00        ; 1x1 (8x8)
EN_B = $01        ; 1x1 (8x8)
EN_C = $02        ; 2x2 (16x16)
EN_D = $03        ; 2x2 (16x16)
...
.segment "HEADER"
  ...
  .byte $02          ; Byte 4: PRG ROM size / 2 × 16KB PRG (32KB)
...
player_blink_timer: .res 1   ; 0=off, >0 frames remaining
...
CatchKindParams:
  .byte $00,$00   ; L1: 0% fractured, 0% final
  .byte $10,$00   ; L2: ~6% fractured
  .byte $20,$00   ; L3: ~12% fractured

  .byte $60,$02   ; L4: ~37% fractured, ~1% final
  .byte $70,$04   ; L5: ~44% fractured, ~2% final
  .byte $80,$06   ; L6: ~50% fractured, ~2% final
  .byte $90,$08   ; L7: ~56% fractured, ~3% final
  .byte $A0,$0A   ; L8: ~63% fractured, ~4% final
  .byte $B0,$0C   ; L9: ~69% fractured, ~5% final

  .byte $D0,$18   ; L10: ~81% fractured, ~9% final
  .byte $E0,$20   ; L11: ~88% fractured, ~12% final
  .byte $F0,$28   ; L12: ~94% fractured, ~16% final
...
@title_run:
  ...
  beq @no_start
    ...
    jsr ResetRun
    jsr EnterIntro
    ; IMPORTANT: don't keep running title logic after changing state
    jsr TextQ_End
    jsr BuildOAM
    jmp MainLoop
...
@go_want_hidden:
  ...
  ; hide: queue blanks (fill with tile $00)
  lda #$00
  sta tmp0
...
; returns A = tile to spawn ($30 core, $31 fractured, $32 final)
; returns A = tile ($30 core, $31 fractured, $32 final)
; returns A = tile ($30/$31/$32)
ChooseCatchKind:
  jsr NextRand
  ; decorrelation like enemy spawn:
  eor rng_hi
  clc
  adc frame_lo
...
@core:
  lda #$30          ; CATCH_TILE_CORE
  rts

@fract:
  lda #$31          ; fractured core tile
  rts
...
; ------------------------------------------------------------
; TextQ_QueueFill
; - Queues a VRAM write of LEN copies of TILE
; - Inputs:
;     A = PPUADDR hi
;     X = PPUADDR lo
;     Y = len
;     tmp0 = tile to repeat (set by caller)
; ------------------------------------------------------------
TextQ_QueueFill:
  sta textq_hi
  stx textq_lo
...
; ------------------------------------------------------------
; TextQ_Reset
; - clears queue indices so NMI has nothing to flush
; ------------------------------------------------------------
TextQ_Reset:
  lda #$00
  sta textq_r
...
BossShotSfxMaybe:
  lda boss_sfx_cd
  bne @no
  jsr PlaySfxBossShot   ; OR PlaySfxBossShot 
...
@after_player:
  ...
  .if DEBUG_DRAW_TEST
  lda debug_mode        ;
  ; e.g. if debug_mode != 0 skip drawing bullets
  bne @skip_bullets
.endif
